...
- **Single lot size.** The lot is always 4 rows × 10 cols regardless of viewport — the same puzzle on desktop and mobile. To keep mobile usable, the renderer starts at zoom 2 when `(hover: none) and (pointer: coarse)` matches, so the car and nearby slots are large enough to maneuver around on a phone.
