...
export class Renderer {
  ...
  render(game: Game): void {
    ...
    let focusX = lerp(game.lot.width / 2, game.car.position.x, followStrength);
    let focusY = lerp(game.lot.height / 2, game.car.position.y, followStrength);
    ...
    for (const slot of game.lot.slots) drawSlot(ctx, slot, slot === game.lot.targetSlot);

    for (const p of game.lot.parkedCars) drawParked(ctx, p);

    drawEntrance(ctx, game.lot.entrance);
    drawPlayerCar(ctx, game.car);
...
function drawPlayerCar(ctx: CanvasRenderingContext2D, car: Car): void {
  ...
  // rear lights — white while reversing, red otherwise (brighter when braking)
